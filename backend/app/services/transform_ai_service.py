...
class TransformAdvisorService(TransformAIBase):
    ...
    def advise(self, user_prompt: str, transformation_summary: dict, language: str = "en") -> dict:
        """
            Farm-level transformation advisory.
            transformation_summary now includes farm_context + experiences_by_level.
        """
    ...
    def advise_experience(self, user_prompt: str, experience_details: dict, farm_context: dict = None, language: str = "en") -> dict:
        """
            Single experience advisory.
            Now accepts farm_context for personalized, budget-aware guidance.
        """
        
        system = experience_advisor_system_prompt(language)
        user = experience_advisory_prompt(user_prompt, experience_details, farm_context)
...
class TransformStoryService(TransformAIBase):
    ...
    def generate_story(self, experience_details: dict, farm_context: dict = None, language: str = "en") -> dict:
        """
            Generate visitor-facing story.
            Now accepts farm_context for culturally authentic storytelling.
        """
        system = story_generator_system_prompt(language)
        user = story_generation_prompt(experience_details, farm_context)
